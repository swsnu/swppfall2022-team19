...
const initialState: ReviewState = {
    reviews: [
        {id: 1, user_id: 1, product_id: 1, score: [4, 3, 5, 3, 3]}
    ],
    selectedReview: null
}
...
export const postReview = createAsyncThunk(
    "api/postReview",
   async (review: Pick<ReviewType, "user_id"|"product_id"|"score">,{dispatch}) => {
...
export const reviewSlice = createSlice({
    ...
    initialState,
    reducers: {
        addReview: (state, action: PayloadAction<{user_id: number, product_id: number, score: number[]}>) =>{
            const newReview = {
