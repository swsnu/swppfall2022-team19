...
class BlogTestCase(TestCase):
    ...
    def test_product_list(self):
        response = self.client.get(
            '/api/product/', {'user_id': 'user_id'}, content_type='application/json')
        self.assertEqual(response.status_code, 200)

    # (O)list /api/product/mainCategory/
    def test_product_mainCategory(self):
        response = self.client.get(
            '/api/product/mainCategory/', {'mainCategory': '간편식사'}, content_type='application/json')
        self.assertEqual(response.status_code, 200)
    ...
    def test_product_update_type_correct(self):
        response = self.client.put(
            '/api/product/1/', {'averageScore': '1.0'}, content_type='application/json')
        self.assertEqual(response.status_code, 200)
        response = self.client.put(
            '/api/product/2/', {'averageScore': '1.0'}, content_type='application/json')
        self.assertEqual(response.status_code, 200)

    def test_product_update_type_wrong(self):
        response = self.client.put(
            '/api/product/1/', {'averageScore': '형식오류1'}, content_type='application/json')
        self.assertEqual(response.status_code, 400)  # input format error
        response = self.client.put(
            '/api/product/2/', {'averageScore': '형식오류2'}, content_type='application/json')
        self.assertEqual(response.status_code, 400)  # input format error
    ...
    def test_rate_list(self):
        ...
        response = self.client.get(
            '/api/rate/user/', {'user_id': '1'}, content_type='application/json')
        self.assertEqual(response.status_code, 200)

        # (O)list /api/rate/liked/
        response = self.client.get(
            '/api/rate/liked/', {'user_id': '1'}, content_type='application/json')
        self.assertEqual(response.status_code, 200)
    ...
    def test_rate_create(self):
        response = self.client.post('/api/rate/',{
            ...
            "picture": "https://tqklhszfkvzk6518638.cdn.ntruss.com/product/8809196616970.jpg"
            }) 
        #print(response.json())
